...
class Group
{
	private static function header($title = '', $up_icon = false)
	{
		...
		$min_item_message = __('You cant remove this item. At least one item needed.', 'easy-invoice');
	...
	public static function render($field, $field_id, $value, $group_id = null)
	{
		...
		if ($repeatable) {
			...
			$item_title_field = isset($field['item_title_field']) ? sanitize_text_field($field['item_title_field']) : 'item_title';
